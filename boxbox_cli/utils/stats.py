...
def _get_executor() -> ThreadPoolExecutor:
    """Lazy initialization of a shared ThreadPoolExecutor for background tasks."""
    global _EXECUTOR
    with _CACHE_LOCK:
        if _EXECUTOR is None:
            _EXECUTOR = ThreadPoolExecutor(max_workers=3, thread_name_prefix="BoxBoxBackground")
        return _EXECUTOR

def shutdown_background_tasks() -> None:
    """Shut down the background executor gracefully."""
    global _EXECUTOR
    with _CACHE_LOCK:
...
def _aggregate_season_stats(year: int, *, basic_only: bool = True):
    ...
    # Per-year aggregation lock to prevent multiple threads from doing the same heavy work
    if not hasattr(_aggregate_season_stats, "_locks"):
        _aggregate_season_stats._locks = defaultdict(threading.Lock)
    
    with _aggregate_season_stats._locks[year]:
        with _CACHE_LOCK:
        ...
        for _, row in schedule.iterrows():
            ...
            for code in to_consider:
                ...
                try:
                    ...
                    if not basic_only:
                        try:
                            # Use getattr to be safe and handle DataNotLoadedError if it occurs
...
def _ensure_session_summary(year: int, rnd: int, code: str) -> Optional[dict]:
    ...
    try:
        ...
        # FastF1 can sometimes fail to load data but not raise an exception immediately.
        # Accessing 'laps' after load() should be safe if load was successful.
        # However, if data is missing from the backend, it might raise DataNotLoadedError.
        try:
            laps = getattr(s, "laps", None)
        except Exception as e:
            _log(f"[summary] WARN: laps not available for {year} rnd={rnd} {code}: {e}")
            laps = None
...
def prewarm_driver_enrichment(year: int, abbrs: list[str]) -> None:
    ...
    for (rnd, code, _) in subset:
        with _CACHE_LOCK:
            already_done = (rnd, code) in processed
        if not already_done:
...
def complete_season_summaries(year: int) -> None:
    ...
    try:
        ...
        futs = [executor.submit(_ensure_session_summary, year, r, c) for (r, c) in to_process]
        for f in futs:
            try: f.result()
